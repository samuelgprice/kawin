...
PROBE_POINT = np.array([0.16, 0.60], dtype=np.float64)
...
TWO_PHASE_NODES = 201
INTERFACE_POSITION = 198.0e-6 + 1.0e-12
...
B_BULK = np.array([0.05, 0.675], dtype=np.float64) # np.array([0.001, 0.625], dtype=np.float64) #np.array([0.001, 0.6], dtype=np.float64)
...
def _surrogate_diffusivity_sampling_kwargs():
    ...
    if bulk_points is None and interpolation in {"nearest", "simplex_linear"}:
        ...
        points_toSkip = np.array([[0.14, 0.06],
                                [0.18, 0.08],
                                [0.24, 0.1 ],
                                [0.26, 0.2 ],
                                [0.28, 0.12],
...
# import plotly
# plotly.offline.plot(results_plot["fig"], filename = fr"C:\Users\samth\OneDrive - Northwestern University\WS_DL\Lab Data\Price\code\kawin\examples\ternaryExamples\CuNiTi_{TEMPERATURE}Kresults_plot.html", auto_open=False)
...
bulk_diff_fig = surrogate_diagnostics["ab"]["figures"]["bulk_diffusivity"][PHASE_A]
bulk_diff_fig.show()
# import plotly
# plotly.offline.plot(bulk_diff_fig, filename = fr"C:\Users\samth\OneDrive - Northwestern University\WS_DL\Lab Data\Price\code\kawin\examples\ternaryExamples\CuNiTi_{TEMPERATURE}K_bulk_diffusivity_ab_{PHASE_A}_surrDiag.html", auto_open=False)
