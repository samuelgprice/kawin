# %%
"""
Fe-Cr-Ni two-phase vs symmetric three-phase Illingworth comparison.

This example compares

    two phase:        BCC_A2 | FCC_A1
    three phase:      BCC_A2 | FCC_A1 | BCC_A2

under a mirror-symmetric construction.  The left half of the three-phase
problem is intended to be the same physical problem as the two-phase problem.

If the two-phase domain is [0, R] with interface s, the symmetric three-phase
domain is [0, 2R] with interfaces

    s_AB = s
    s_BA = 2R - s.

Thus the FCC middle phase in the three-phase problem is initially twice as
wide as the FCC phase in the two-phase problem.  The center x=R is a symmetry
plane and should reproduce the zero-flux outer boundary of the two-phase FCC
region.

The comparison is deliberately run with the same fixed diffusivity matrices
sampled from the Fe-Cr-Ni CALPHAD thermodynamics.  This isolates the numerical
two-phase/three-phase reduction from differences caused by independently
evaluating composition-dependent diffusivities.

The script:
  1. builds the Fe-Cr-Ni BCC/FCC tie-line surrogate used by
     IllingworthTernaryExamples.py,
  2. samples one fixed 2x2 diffusivity matrix per phase,
  3. constructs matched two- and three-phase meshes/transformed grids,
  4. solves both problems,
  5. plots interface, eta, profile, symmetry, and conservation comparisons,
  6. prints quantitative discrepancy metrics.

The most important expected relations are

    s_3,left(t)          ~= s_2(t)
    2R - s_3,right(t)   ~= s_2(t)
    (s_3,right-s_3,left)/2 ~= R-s_2(t)
    eta_3,left(t)        ~= eta_2(t)
    eta_3,right(t)       ~= eta_2(t)

and the full three-phase composition profile should remain mirror symmetric
about x=R.

This is a consistency/reduction benchmark, not an independent validation of
the physical model.  Grid/time convergence and comparison with an independent
solver (e.g. DICTRA) are still separate validation steps.
"""
...
PROBE_POINT = np.array([0.40, 0.16], dtype=np.float64)
...
BCC_BULK = np.array([0.38, 0.001], dtype=np.float64)
FCC_BULK = np.array([0.13, 0.15], dtype=np.float64)
